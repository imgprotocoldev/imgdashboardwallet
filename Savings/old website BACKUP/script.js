...
class SidebarLoader {
    ...
    // Update premium status for wallet connection
    updatePremiumStatus(isPremium) {
        console.log('🔄 Updating sidebar premium status:', isPremium);
        
        // Always enable dashboard
        const dashboardItem = document.querySelector('.nav-item[data-page="dashboard"]');
        if (dashboardItem) {
            dashboardItem.classList.remove('disabled');
            dashboardItem.style.opacity = '1';
            dashboardItem.style.cursor = 'pointer';
            dashboardItem.style.pointerEvents = 'auto';
        }
        
        // Update premium nav items
        ...
        premiumItems.forEach(item => {
            if (isPremium) {
                // Enable premium items
                item.classList.remove('disabled');
                item.style.opacity = '1';
                item.style.cursor = 'pointer';
                item.style.pointerEvents = 'auto';
                console.log(`✅ Enabled premium item: ${item.getAttribute('data-page')}`);
            } else {
                // Disable premium items
                item.classList.add('disabled');
                item.style.opacity = '0.5';
                item.style.cursor = 'not-allowed';
                item.style.pointerEvents = 'none';
                console.log(`❌ Disabled premium item: ${item.getAttribute('data-page')}`);
            }
        });
        
        console.log('🔄 Sidebar premium status updated - Dashboard always available, Premium items:', isPremium ? 'ENABLED' : 'DISABLED');
    }
    
    checkWalletStateForPremium() {
        // PAGE-LEVEL WALLET STATUS: Check if current page has wallet connected
        const pageWalletStatus = this.getPageWalletStatus();
        
        if (pageWalletStatus.isConnected) {
            console.log('🚀 PAGE WALLET: Premium access enabled from page status');
            this.enablePremiumMenu();
        } else {
            console.log('🔒 PAGE WALLET: No wallet connected on this page');
            this.disablePremiumMenu();
        }
    }
    
    getPageWalletStatus() {
        // Check if current page has wallet status set
        const pageWallet = window.pageWalletStatus || {};
        
        // Validate wallet status with fallback to localStorage
        let isConnected = pageWallet.isConnected || false;
        let publicKey = pageWallet.publicKey || null;
        let isPremium = pageWallet.isPremium || false;
        
        // Fallback: If page status is missing, check localStorage
        if (!pageWallet.isConnected && !pageWallet.publicKey) {
            const storedWallet = localStorage.getItem('imgProtocolWalletState');
            if (storedWallet) {
                try {
                    const parsed = JSON.parse(storedWallet);
                    isConnected = parsed.isConnected || false;
                    publicKey = parsed.walletPublicKey || null;
                    isPremium = parsed.isPremium || false;
                    
                    // Update page status for future use
                    if (isConnected) {
                        ...
                        console.log('🔄 SIDEBAR: Wallet status restored from localStorage');
                    }
                } catch (error) {
                    console.error('❌ SIDEBAR: Error parsing stored wallet state:', error);
                }
            }
        }
        
        return { isConnected, publicKey, isPremium };
    }
    
    enablePremiumMenu() {
        // Enable premium menu items immediately
        const premiumItems = document.querySelectorAll('.nav-item[data-page="harvesting"], .nav-item[data-page="distribution"], .nav-item[data-page="wallet-lookup"], .nav-item[data-page="reward-calculator"]');
        ...
        console.log('✅ PREMIUM MENU: All premium items enabled');
    }
    
    disablePremiumMenu() {
        // Disable premium menu items
        const premiumItems = document.querySelectorAll('.nav-item[data-page="harvesting"], .nav-item[data-page="distribution"], .nav-item[data-page="wallet-lookup"], .nav-item[data-page="reward-calculator"]');
        ...
        console.log('🔒 PREMIUM MENU: All premium items disabled');
    }
    
    setupWalletStatusListener() {
        // Listen for wallet status changes from wallet manager
        window.addEventListener('pageWalletStatusChanged', (event) => {
            console.log('📡 SIDEBAR: Wallet status changed:', event.detail);
        ...
        console.log('📡 SIDEBAR: Wallet status listener setup complete');
    }
    
    forcePremiumStateBeforeSidebarLoad() {
        // Set premium state before sidebar elements are even created
        const premiumPages = ['harvesting', 'distribution', 'wallet-lookup', 'reward-calculator'];
        
        // Create a MutationObserver to watch for new sidebar elements
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (mutation.type === 'childList') {
                    mutation.addedNodes.forEach((node) => {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            // Check if this is a premium nav item
                            premiumPages.forEach(page => {
                                const items = node.querySelectorAll ? node.querySelectorAll(`[data-page="${page}"]`) : [];
                                items.forEach(item => {
                                    // Immediately enable premium items as they're created
                                    item.classList.remove('disabled');
                                    item.style.opacity = '1';
                                    item.style.cursor = 'pointer';
                                    item.style.pointerEvents = 'auto';
                                    console.log(`🚀 INSTANT ENABLE: ${page} enabled as it loads`);
                                });
                            });
                        }
                    });
        ...
        if (this.sidebarContainer) {
            ...
            console.log('🚀 INSTANT SIDEBAR: MutationObserver watching for premium items');
        }
        
        // Also force update existing elements
        setTimeout(() => {
            this.updatePremiumStatus(true);
        }, 50);
    }
    
    forcePremiumStateAfterSidebarLoad() {
        // Force premium state after sidebar is fully loaded
        const hasWallet = localStorage.getItem('walletConnected') === 'true' || 
                         localStorage.getItem('imgProtocolWalletState');
        
        if (hasWallet) {
            console.log('🚀 POST-LOAD PREMIUM: Forcing premium state after sidebar load...');
            
            // Force enable all premium items
            const premiumItems = document.querySelectorAll('.nav-item[data-page="harvesting"], .nav-item[data-page="distribution"], .nav-item[data-page="wallet-lookup"], .nav-item[data-page="reward-calculator"]');
            ...
            console.log('✅ POST-LOAD PREMIUM: All premium items forced enabled');
        }
    }
    ...
    async init() {
        ...
        try {
            ...
            console.log('Sidebar initialization complete!');
                    ...
                    // PAGE-LEVEL WALLET STATUS: Check wallet status from page
        this.checkWalletStateForPremium();
        
        // Listen for wallet status changes from wallet manager
        this.setupWalletStatusListener();
        
        // Also force premium state after sidebar loads
        setTimeout(() => {
            this.forcePremiumStateAfterSidebarLoad();
        }, 100);
    ...
    setupSidebar() {
        ...
        navLinks.forEach(link => {
            link.addEventListener('click', (e) => {
                ...
                // REMOVED: Mobile menu close to eliminate conflicts
            });
        });
        
        // REMOVED: Mobile menu functionality to eliminate conflicts
        
        // REMOVED: Conflicting premium page disabling
        
        console.log('Enhanced sidebar setup complete - Clean Premium Menu Management');
    }
    ...
    // REMOVED: Conflicting premium page disabling that overrides wallet state
    ...
    navigateToPage(targetPage) {
        ...
        if (targetUrl) {
            ...
            // PRESERVE WALLET STATE: Save current state before navigation
            if (window.walletManager && window.walletManager.isConnected) {
                console.log('💾 Preserving wallet state before navigation...');
                window.walletManager.saveWalletState();
            }
    ...
    // REMOVED: Mobile sidebar initialization to eliminate conflicts
    
    // REMOVED: Mobile layout forcing to eliminate conflicts
    
    // REMOVED: Resize handling to eliminate conflicts
    
    // REMOVED: Mobile device detection to eliminate conflicts
    
    // REMOVED: Mobile menu toggle methods to eliminate conflicts
    ...
    // REMOVED: Mobile button setup methods to eliminate conflicts
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Force mobile layout check after initialization
    setTimeout(() => {
        // REMOVED: Mobile layout check to eliminate conflicts
        if (window.sidebarLoader) {
            console.log('✅ SIDEBAR: All mobile conflicts removed - clean wallet state management');
        }
    }, 1000);
});
...
class SolanaWalletManager {
    constructor() {
        ...
        this.tokenAmount = 0;
        this.walletStatePersisted = false;
        this.isRestoringWalletState = false;
        
        this.init();
    }
    
    init() {
        // FAST INIT: Single localStorage check, immediate UI update
        const hasWallet = localStorage.getItem('walletConnected') === 'true' || 
                         localStorage.getItem('imgProtocolWalletState');
        
        if (hasWallet) {
            // FAST UI UPDATE: No delays, immediate premium access
            this.updateWalletUI();
            this.updatePremiumStatus();
            console.log('🚀 FAST INIT: Premium access enabled immediately');
        }
    ...
    // Universal Wallet State Management
    saveWalletState() {
        try {
            const walletState = {
            ...
            localStorage.setItem('imgProtocolWalletState', JSON.stringify(walletState));
            
            // Also set a flag for immediate access
            localStorage.setItem('walletConnected', this.isConnected ? 'true' : 'false');
            localStorage.setItem('walletPremium', this.isPremium ? 'true' : 'false');
    ...
    broadcastWalletState() {
        ...
        console.log('📡 Wallet state broadcasted to all components');
    }
    
    loadPersistedWalletState() {
        try {
            const savedState = localStorage.getItem('imgProtocolWalletState');
            if (savedState) {
                const walletState = JSON.parse(savedState);
                
                // Restore wallet state
                this.isConnected = walletState.isConnected;
                this.walletPublicKey = walletState.walletPublicKey;
                this.isPremium = walletState.isPremium;
                this.tokenAmount = walletState.tokenAmount;
                
                console.log('📱 Restored wallet state:', walletState);
                
                // Update UI immediately if wallet was connected
                if (this.isConnected && this.walletPublicKey) {
                    this.updateWalletUI();
                    this.updatePremiumStatus();
                    
                    // Set page-level wallet status for sidebar
                    this.setPageWalletStatus();
                }
            }
        } catch (error) {
    ...
    setPageWalletStatus() {
        // Set page-level wallet status that sidebar can read instantly
        window.pageWalletStatus = {
            isConnected: this.isConnected,
        ...
        console.log('🚀 PAGE WALLET STATUS SET:', window.pageWalletStatus);
        
        // Also dispatch event for immediate sidebar update
        const event = new CustomEvent('pageWalletStatusChanged', {
            detail: window.pageWalletStatus
        });
        window.dispatchEvent(event);
    }
    
    clearPageWalletStatus() {
        // Clear page-level wallet status
        window.pageWalletStatus = {
            isConnected: false,
            publicKey: null,
            isPremium: false,
        ...
        console.log('🔒 PAGE WALLET STATUS CLEARED');
    ...
    updatePremiumStatus() {
        // FAST PREMIUM MENU: Single check, immediate enable/disable
        const premiumItems = document.querySelectorAll('.nav-item[data-page="harvesting"], .nav-item[data-page="distribution"], .nav-item[data-page="wallet-lookup"], .nav-item[data-page="reward-calculator"]');
        
        // Single, fast check: has wallet ever been connected?
        const hasWallet = localStorage.getItem('walletConnected') === 'true' || 
                         localStorage.getItem('imgProtocolWalletState');
        
        if (hasWallet) {
            // FAST ENABLE: All premium items immediately
            premiumItems.forEach(item => {
                item.classList.remove('disabled');
                item.style.opacity = '1';
                item.style.cursor = 'pointer';
            ...
            console.log('🚀 FAST PREMIUM: All items enabled in one operation');
        } else {
            // FAST DISABLE: All premium items immediately
            premiumItems.forEach(item => {
                item.classList.add('disabled');
                item.style.opacity = '0.5';
                item.style.cursor = 'not-allowed';
            ...
            console.log('🔒 FAST PREMIUM: All items disabled in one operation');
        }
    }
    ...
    async connectWallet(provider) {
        try {
            ...
            if (hasImgTokens) {
                ...
                // Set PAGE-LEVEL wallet status (this is the key!)
                this.setPageWalletStatus();
    ...
    async checkImgTokens() {
        try {
            ...
            this.tokenAmount = imgTokenBalance;
            this.isPremium = hasEnoughTokens;
            
            // Log the decision
            console.log(`Wallet ${this.walletPublicKey}: ${hasEnoughTokens ? 'GRANTED' : 'DENIED'} access (${this.tokenAmount.toLocaleString()} IMG tokens)`);
            console.log(`💡 IMG Token Address: znv3FZt2HFAvzYf5LxzVyryh3mBXWuTRRng25gEZAjh`);
    ...
    async disconnectWallet() {
        try {
            ...
            // Clear saved wallet state from localStorage
            localStorage.removeItem('imgProtocolWalletState');
            console.log('🗑️ Cleared saved wallet state from localStorage');
            
            // REMOVED: Conflicting premium access disabling
            
            // Clear page-level wallet status
            this.clearPageWalletStatus();
    ...
    updateUI() {
        // SMART BUTTON STATE: Preserve button state during navigation
    ...
    // Smart connect button state management
    updateConnectButton() {
        const connectBtn = document.getElementById('connect-wallet-btn');
        if (!connectBtn) return;
        
        // Check if we should preserve existing button state
        const shouldPreserveState = this.shouldPreserveButtonState();
        
        if (shouldPreserveState) {
            console.log('🔄 Preserving existing button state during navigation...');
            return; // Don't change button during navigation
        }
        ...
        if (this.isConnected) {
            ...
            connectBtn.style.background = 'linear-gradient(135deg, rgba(220, 38, 38, 0.15) 0%, rgba(185, 28, 28, 0.15) 100%)';
            connectBtn.style.borderColor = 'rgba(239, 68, 68, 0.3)';
            console.log('🔴 Button updated to: DISCONNECT WALLET');
        } else {
            connectBtn.innerHTML = '<span class="nav-text connect-wallet-text">CONNECT WALLET</span>';
            connectBtn.style.background = '';
            connectBtn.style.borderColor = '';
            console.log('🔵 Button updated to: CONNECT WALLET');
        }
    }
    
    // Determine if we should preserve button state during navigation
    shouldPreserveButtonState() {
        // If we're in the middle of restoring wallet state, preserve button
        if (this.isRestoringWalletState) {
            return true;
        }
        
        // If we have a persisted state and button is already correct, preserve it
        const connectBtn = document.getElementById('connect-wallet-btn');
        if (connectBtn) {
            const currentText = connectBtn.textContent.trim();
            const expectedText = this.isConnected ? 'DISCONNECT WALLET' : 'CONNECT WALLET';
    ...
    checkExistingConnection() {
        ...
        // SMART WALLET STATE: Preserve existing state if valid
        const existingState = this.getPersistedWalletState();
        ...
        if (window.solana && window.solana.isConnected && window.solana.publicKey) {
            ...
            shouldVerify = true;
        } else if (existingState && existingState.isConnected) {
            console.log('🔄 Restoring persisted wallet state...');
            this.isConnected = existingState.isConnected;
            this.walletPublicKey = existingState.walletPublicKey;
            this.isPremium = existingState.isPremium;
            
            // Update UI immediately with persisted state
            this.updateUI();
            return; // Skip verification, use persisted state
        } else {
        ...
        // Only verify if we have a live connection
        if (shouldVerify) {
            this.verifyWalletConnection();
        }
    }
    
    // Get persisted wallet state without resetting current state
    getPersistedWalletState() {
        try {
            const walletConnected = localStorage.getItem('walletConnected') === 'true';
            const storedState = localStorage.getItem('imgProtocolWalletState');
            
            if (walletConnected && storedState) {
            ...
            console.error('Error reading persisted wallet state:', error);
            return null;
        }
    }
    
    // Save wallet state for navigation preservation
    saveWalletState() {
        try {
            const state = {
                isConnected: this.isConnected,
            ...
            console.log('💾 Wallet state saved for navigation:', state);
        } catch (error) {
            console.error('Error saving wallet state:', error);
        }
    }
    
    // REMOVED: Conflicting force disable method that overrides wallet state
    ...
    async verifyWalletConnection() {
        try {
            ...
            // Don't disconnect on verification error - keep existing state
            console.log('🔄 Keeping existing wallet state due to verification error');
        }
    }
